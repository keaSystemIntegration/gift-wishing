...
inviteRoutes.post('/send', async (req, res) => {
    try {
        ...
        console.log(req.cookies.Claims)
        if (!isEmail(friendEmail)) {
            throw new Error('Invalid friendEmail')
        }
        const inviteSuccess = await inviteService.sendInvitation(
            userId,
            friendEmail
        )
        if (inviteSuccess === true) {
            ...
            res.status(503).json({
                inviteStatus: 'Error! Please try again later!',
            })
        }
    } catch (e) {
...
inviteRoutes.post('/accept', async (req, res) => {
    try {
        ...
        const acceptedResponse = await inviteService.acceptInvitation(
            userId,
            token,
            name,
            username,
        ...
        if (acceptedResponse === 'INVALID TOKEN') {
            res.status(403).send({
                error: "Invalid token - token might be too old, although that'd be weird, no expiration on tokens here!",
            })
        } else if (acceptedResponse === 'INVALID EMAIL') {
            res.status(403).send({
                error: 'Invalid email, does not match invite token!',
            })
        } else {
